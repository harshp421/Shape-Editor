...
const pagesReducer: Reducer<PAGES, PAGE_ACTION> = function (
  ...
  function updateSnapshotOfCurrentPage() {
    const currentPageSnapshot =
      document.getElementById("svgEditor")?.innerHTML!;
  ...
  switch (action.type) {
    case PAGES_ACTION_TYPES.SET_ACTIVE_PAGE: {
      return {
        ...state,
        history: {
          past: [...state.history.past, state.pages],
          present: state.pages.map((page, index) =>
            index === state.activePageIndex
              ? { ...page, activePageIndex: action.payload }
              : page
          ),
          future: [],
        },
    ...
    case PAGES_ACTION_TYPES.UNDO_SELECTED_SHAPES:
      if (state.history.past.length === 0) return state; // No past to undo to
    ...
    case PAGES_ACTION_TYPES.REDU_SELECTED_SHAPES:
      if (state.history.future.length === 0) return state; // No future to redo to
    ...
    case PAGES_ACTION_TYPES.TOGGLE_CONTEXT_MENU: {
      state.contextMenu = action.payload
        ? {
            show: true,
            clipboard: { ...state.contextMenu.clipboard },
            ...action.payload,
          }
        : {
            show: false,
    ...
    case PAGES_ACTION_TYPES.CUT_SELECTED_SHAPES: {
      ...
      state.contextMenu = {
        ...state.contextMenu,
        clipboard: { x: state.contextMenu.x, y: state.contextMenu.y },
      };
    ...
    case PAGES_ACTION_TYPES.PASTE_SELECTED_SHAPES: {
      ...
      const deepCloneGroup = function (shape: GROUP_SHAPE, pageIndex: number) {
        for (let i = 0; i < shape.children.length; i++) {
          ...
          const newChildShape = cloneDeep(
            state.pages[pageIndex].shapes[childShapeId]
          );
          ...
          if (newChildShape.type === SHAPE_TYPES.GROUP) {
            const pageIndex = state.pages.findIndex(
              (page) => page.id === newChildShape.pageId
            );
      ...
      state.clipboard.forEach((item) => {
        ...
        newShape.style.translate = [
          item.style.translate[0] + dx,
          item.style.translate[1] + dy,
        ];
        shapesToPaste[newShape.id] = newShape;
        if (newShape.type === SHAPE_TYPES.GROUP) {
          const pageIndex = state.pages.findIndex(
            (page) => page.id === newShape.pageId
          );
    ...
    case PAGES_ACTION_TYPES.REMOVE_SVG_FILTER: {
      ...
      svgFilters[action.payload.filterType] = svgFilters[
        action.payload.filterType
      ]?.filter((filterId) => filterId !== action.payload.filterId);
    ...
    case PAGES_ACTION_TYPES.SCALE_ACTIVE_SHAPE: {
      ...
      activeShapes.forEach((shapeId) => {
        ...
        s.style.scale[0] += dx < 0 ? -0.15 : dx > 0 ? 0.15 : 0;
        s.style.scale[1] += dy < 0 ? -0.15 : dy > 0 ? 0.15 : 0;
        shapes[shapeId] = { ...s };
      });
